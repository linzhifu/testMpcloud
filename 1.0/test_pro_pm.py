...
def addRole(user, wait, driver):
    ...
    addRoleBtn = wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, '.add-role-btn')),
        message='找不到 添加角色按键')
    ...
    # 添加用户按钮
    addUserBtn = wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, '.add-user-btn')),
        message='找不到 添加用户按键')
...
def searchPro(user, wait):
    ...
    if getTeamInfo(wait, 3, 2) != user['modName']:
        raise Exception(user['modName'] + ' 显示错误：' + getTeamInfo(wait, 3, 2))
    if getTeamInfo(wait, 3, 3) != user['modDes']:
        raise Exception(user['modName'] + ' 描述显示错误：' + getTeamInfo(wait, 3, 3))
...
def proList(user, wait, driver):
    if user['proLisEnable']:
        ...
        # 测试产品型号功能：添加、修改、删除
        logging.info('测试产品类型功能(添加、修改、删除)')
...
def orderTools(user, wait, driver):
    if user['orderToolEnable']:
        ...
        orderToolsBtn = wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR,
                                        'ul:nth-child(1)>li:nth-child(4) ul')),
...
def main(driver, user=config.USER_PRO_PM):
    # user :USER_PRO_PM
