...
def searchPro(user, wait):
    ...
    # 检查订单显示
    # ORDER1
    if getTeamInfo(wait, 1, 1) != config.ORDER_1['NUM']:
    ...
    if getTeamInfo(wait, 1, 6) != config.ORDER_1['TIME']:
        raise Exception(config.ORDER_1['TIME'] + ' 有效时间显示错误：' +
    ...
    # ORDER2
    if getTeamInfo(wait, 2, 1) != config.ORDER_2['NUM']:
    ...
    if getTeamInfo(wait, 2, 6) != config.ORDER_2['TIME']:
        raise Exception(config.ORDER_2['TIME'] + '有效时间显示错误：' +
...
def addOrderTool(user, wait):
    ...
    if getTeamInfo(wait, 1, 4) != config.ORDER_1['FACTORY']:
        raise Exception(config.ORDER_1['FACTORY'] + ' 工厂错误：' +
                        getTeamInfo(wait, 1, 4))
    if getTeamInfo(wait, 1, 6) != config.ORDER_1['TIME']:
        raise Exception(config.ORDER_1['TIME'] + ' 有效时间错误：' +
                        getTeamInfo(wait, 1, 6))
...
def searchByOrderNum(wait):
    ...
    if getTeamInfo(wait, 1, 5) != config.ORDER_1['FACTORY']:
        raise Exception(config.ORDER_1['FACTORY'] + ' 工厂不对：' +
                        getTeamInfo(wait, 1, 5))
    if getTeamInfo(wait, 1, 6) != config.ORDER_1['TIME']:
        raise Exception(config.ORDER_1['TIME'] + ' 有效时间不对：' +
                        getTeamInfo(wait, 1, 6))
...
def searchAllOrder(wait):
    ...
    if getTeamInfo(wait, 2, 1) != config.ORDER_2['NUM']:
        raise Exception(config.ORDER_2['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 2, 1))
    if getTeamInfo(wait, 3, 1) != config.ORDER_3['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 3, 1))
    if getTeamInfo(wait, 4, 1) != config.ORDER_4['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 4, 1))
    if getTeamInfo(wait, 5, 1) != config.ORDER_5['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 5, 1))
    if getTeamInfo(wait, 6, 1) != config.ORDER_6['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 6, 1))
    if getTeamInfo(wait, 7, 1) != config.ORDER_7['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
                        getTeamInfo(wait, 7, 1))
    if getTeamInfo(wait, 8, 1) != config.ORDER_8['NUM']:
        raise Exception(config.ORDER_1['NUM'] + '订单号不对：' +
...
def orderList(user, wait, driver):
    if user['orderListEnable']:
        ...
        orderListsBtn = wait.until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR,
                 'li.el-submenu:nth-of-type(4) li.el-menu-item:nth-of-type(3)'
                 )),
            message='找不到 订单列表按键')
...
def addSampleInfo(wait, user, sample):
    ...
    waferInput = wait.until(
        ...
        message='找不到 容量输入栏')
    waferInput[0].send_keys(sample['WAFER'][0])
    sleep(time)
    ...
    for i in range(len(wafers)):
        if sample['WAFER'][1] == wafers[i].text:
            wafers[i].click()
            sleep(time)
            break

        if i == len(wafers) - 1:
            raise Exception('容量不存在：' + sample['WAFER'][1])
    ...
    for i in range(len(subs)):
        if sample['SUB'] == subs[i].text:
            subs[i].click()
            sleep(time)
            break

        if i == len(subs) - 1:
            raise Exception('基板不存在：' + sample['SUB'])
    ...
    ngInput = wait.until(
        EC.presence_of_all_elements_located((By.CSS_SELECTOR,
                                             '.extra-value input')),
        message='找不到 不良描述输入栏')
    ngInput[0].send_keys(sample['NG'])

    # 备注
    ngInput[1].send_keys(sample['NOTE'])
...
def newRecord(wait):
    ...
    testTime = wait.until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, '.seven input')),
        message='找不到 测试时间')
    testTime.click()
    sleep(time)
...
def main(driver, user=config.USER_MOD_TL):
    # user :USER_PRO_PM
    ...
    try:
        ...
        # 屏幕截图
        save_screen = driver.save_screenshot(savePath + '-' + resu + '.png')
        if save_screen:
